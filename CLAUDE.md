...
- **Keep the end-user docs current.** `docs/` (Sphinx, built with `make html` from inside `docs/`, deps in `docs/requirements.txt`) is the help-centre content for families/volunteers/dojo teams — not this file. If a change alters a user-facing flow this documents, update the relevant `docs/source/**/*.rst` page in the same change. It intentionally only documents flows that are actually wired up end-to-end — see the note in `docs/source/dojo-team/running-a-session.rst` for what to do once dashboard attendance-marking is implemented.
