...
class EngagementStageAttribute(SegmentAttribute):
    ...
    label = "How the child comes to sessions"
    value_type = "choice"
    scope = NINJA
...
class StageAtDojoAttribute(SegmentAttribute):
    ...
    label = "How the child comes to one dojo"
    ...
    def validate(self, operator, value):
        if operator != "in":
            raise ValueError(f"“{self.label}” supports in, not “{operator}”.")
        ...
        if (not isinstance(value, dict) or value.get("dojo") not in {c.value for c in self.dojo_choices()}
            ...
            raise ValueError(f"“{self.label}” needs a dojo and at least one stage.")
    ...
    def describe(self, operator, value):
        ...
        dojo = dict((c.value, c.label) for c in self.dojo_choices()).get(value.get("dojo"), "a dojo")
        return f"At {dojo}: {', '.join(labels.get(s, s) for s in value.get('stages', []))}"
...
class AttendanceRateAttribute(_EngagementAttribute):
    key = "attendance_rate"
    label = "Share of their sessions they came to (last 180 days, %)"
    value_type = "number"
    unit = "%"
...
class SessionsAttendedAttribute(_EngagementAttribute):
    ...
    label = "Sessions they came to in the last 180 days"
    value_type = "number"
    field = "attended_180d"
...
class MissedInARowAttribute(_EngagementAttribute):
    key = "missed_in_a_row"
    label = "Sessions missed in a row"
    value_type = "number"
...
class NoShowsAttribute(_EngagementAttribute):
    key = "no_shows"
    label = "Booked but didn't come (last 90 days)"
    value_type = "number"
    field = "no_shows_90d"
...
class DaysSinceLastVisitAttribute(_EngagementAttribute):
    ...
    label = "Days since their last session"
    value_type = "number"
    unit = " days"
...
class MainDojoStatusAttribute(SegmentAttribute):
    ...
    label = "Status of their main dojo"
    value_type = "choice"
    scope = NINJA
