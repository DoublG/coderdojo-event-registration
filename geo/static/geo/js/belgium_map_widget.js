...
"use strict";
/*
 * Fork of Django's gis/js/OLMapWidget.js that additionally draws a
 * reference overlay of Belgium's country outline and province
 * boundaries (see core/widgets.py:BelgiumOSMWidget), so editors placing
 * a point can see where they are relative to the country/provinces.
 * Kept as a full copy (rather than subclassing) because MapWidget is
 * only reachable through Django's own DOMContentLoaded initializer.
 */
...
class MapWidget {
    ...
    addBoundaryOverlay() {
        // Reference layer: Belgium's outline and province borders, drawn
        // under the editable feature so it never intercepts clicks/drags.
        if (!this.options.boundaries_geojson) {
            return;
        }
        const source = new ol.source.Vector({
            features: new ol.format.GeoJSON().readFeatures(
                this.options.boundaries_geojson,
                {
            ...
            style: (feature) =>
                new ol.style.Style({
                    stroke: new ol.style.Stroke({
                        color:
                            feature.get("kind") === "country"
                                ? "#c0392b"
                                : "#2980b9",
                        width: feature.get("kind") === "country" ? 2 : 1,
                    }),
